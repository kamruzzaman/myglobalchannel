            ...
            <?php
                ...
                $query = $this->db->query("SELECT * FROM `album_tbl` WHERE `album_type` = 'audio' and `user_id` = ".$user_data['user_id']);
                 foreach ($query->result() as $row){
                    ...
                    $cat_id = $row->id;
                     $gallery_query = $this->db->query("SELECT * FROM audiogallery_tbl WHERE album_id = '".$cat_id."'");
                     $tab_content .= '<ul class="gallery clearfix" id="gallery_box">';
                     foreach ($gallery_query->result() as $img_row){
                         $tab_content .= '<li class="g-img"><a href="'.base_url('uploads/audio_Gallery/').$img_row->audio_list.'" class="popup-youtube no-padd"><audio src="'.base_url('uploads/audio_Gallery/').$img_row->audio_list.'" controls style="width: 305px"></audio>
                                                </a></li>';
