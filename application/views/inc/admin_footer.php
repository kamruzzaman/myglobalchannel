	...
	var date_last_clicked = null;
	    ...
	    $(document).ready(function(){
	        notice_tbl = $('#notice_tbl').DataTable({
                'ajax' : base_url+'admin/get_all_notice',
        		'order': [[ 3, 'desc' ]]
            });
            slider_tbl = $('#slider_tbl').DataTable({
                'ajax' : base_url+'admin/get_all_slider',
        		'order': [[ 3, 'desc' ]]
            });
            rotor_tbl = $('#rotor_tbl').DataTable({
                'ajax' : base_url+'admin/get_all_rotor',
        		'order': [[ 3, 'desc' ]]
            });
            category_tbl = $('#category_tbl').DataTable({
                'ajax' : base_url+'admin/get_all_category',
        		'order': [[ 3, 'desc' ]]
            });
            post_tbl = $('#post_tbl').DataTable({
                'ajax' : base_url+'admin/get_all_post',
        		'order': [[ 3, 'desc' ]]
            });
        ...
        function deleteSlider(evt){
        	var id = $(evt).data('id');
        	$.ajax({
        		url: base_url+"admin/delete_Slider/"+id,
        		success:function(response){
        ...
        function deleteRotor(evt){
        	var id = $(evt).data('id');
        	$.ajax({
        		url: base_url+"admin/delete_rotor/"+id,
        		success:function(response){
        			console.log(response);
        ...
        function deletePost(evt){
            var id = $(evt).data('id');
            $.ajax({
        		url: base_url+"admin/delete_post/"+id,
        		success:function(response){
        			console.log(response);
