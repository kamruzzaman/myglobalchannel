...
<div class=" w3-content" style="max-width:1600px">
    ...
    <div class="w3-main">
        ...
        <div id="main" style="width: 100%;float: left;margin-top: 5%">
            <h2 style="text-align: center;padding: 20px">video Gallery</h2>
            <?php
                $tab_menu = '';
                $tab_content = '';
                $i = 0;
                ...
                $query = $this->db->query("SELECT * FROM `album_tbl` WHERE `album_type` = 'video' and `user_id` = ".$user_data['user_id']);
                 foreach ($query->result() as $row){
                     ...
                     $cat_id = $row->id;
                     $gallery_query = $this->db->query("SELECT * FROM videogallery_tbl WHERE album_id = '".$cat_id."'");
                     $tab_content .= '<ul class="gallery clearfix" id="gallery_box">';
                     foreach ($gallery_query->result() as $img_row){
                         $tab_content .= '<li><a href="'.base_url('uploads/video_Gallery/').$img_row->video_list.'" title="Title" class="popup-youtube no-padd"><video src="'.base_url('uploads/video_Gallery/').$img_row->video_list.'" controls style="width: 305px"></a></li>';
